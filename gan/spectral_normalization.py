...
def l2normalize(v, eps=1e-12):
    ...
    return F.normalize(v, eps=eps, dim=0)
...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        for i in range(self.power_iterations):
            # u = torch.matmul(w, v) / l2normalize(torch.matmul(w, v))
            # v = torch.matmul(w, u) / l2normalize(torch.matmul(w, u))
            v = l2normalize(torch.matmul(torch.transpose(w.view(w.size(0),-1), 0, 1), u))
            u = l2normalize(torch.matmul(w.view(w.size(0),-1), v))
        
        w = w / torch.matmul(torch.matmul(u, w.view(w.size(0),-1)), v)
