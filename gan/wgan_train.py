...
def w_train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, c=0.01, n_critic=2, show_every=250,
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            noise = sample_noise(batch_size, noise_size).cuda().detach()
            noise = noise.reshape((batch_size, noise_size, 1, 1))

            fake_images = G.forward(noise).detach()
#             fake_images = fake_images.reshape((batch_size, input_channels, img_size, img_size))

            fake_output = D.forward(fake_images)
            real_output = D.forward(real_images)
            ...
            for p in D.parameters():
                p.data = torch.clamp(p.data, min=-c, max=c)
